...
DBPArchive& DBPArchive::Discard(void* p, size_t sz) { assert(0); return *this; }
	...
	for (size_t i = 0; i != num_ptrs; i++)
	{
		if (mode == MODE_SAVE)
		{
			for (n = 0; n != lutnum; n++)
				if (ptrs[i] == lut[n])
					break;
			assert(ignore_unknown || n != lutnum);
		}
		SerializeByte(&n);
...
void DBPArchive::DoExceptionList(void* p, size_t sz, size_t num_exceptions, ...)
{
	...
	for (va_start(ap, num_exceptions); num_exceptions; num_exceptions--)
	{
		...
		assert(pEx >= pLast && pEx + szEx <= pEnd);
...
DBPArchive& DBPArchiveOptional::SerializeBytes(void* p, size_t sz)
{
	switch (optionality)
	{
		...
		case OPTIONAL_SKIP:      assert(0);                break;
	}
	return *this;
}
...
void DBPSerialize_All(DBPArchive& ar, bool invalid_state)
{
	...
	if (ar.mode == DBPArchive::MODE_LOAD || ar.mode == DBPArchive::MODE_SAVE)
	{
		...
		//if (version != 1) { assert(false); ar.had_error = DBPArchive::ERR_VERSION; ar.error_info = version; return; }
	...
	Bit8u serialized_vgamem = (Bit8u)(vga.mem.memsize / (1024*128)), current_vgamem = serialized_vgamem;
